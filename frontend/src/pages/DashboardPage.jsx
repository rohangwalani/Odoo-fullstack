        ...
        <div className="employee-grid">
          {employees
            ...
            .map((emp) => {
              ...
              const empId = (emp.loginId || emp.id || '').toString();
              const isHighlighted = q && (empName.toLowerCase().includes(q) || empId.toLowerCase().includes(q));
              
              const avatarUrl = emp.profilePicture ? `http://localhost:8080/uploads/${emp.profilePicture}` : null;
              ...
              return (
                ...
                >    <div className="emp-card-avatar" style={{ overflow: 'hidden' }}>
                    ...
                    <div style={{ display: avatarUrl ? 'none' : 'flex', width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <div className="emp-card-overlay">
                    <div className="overlay-info" style={{ textAlign: 'left', marginBottom: '0.5rem' }}>
                      ...
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}><Phone size={12} style={{ display: 'inline', marginRight: '4px' }} /> {emp.phone || 'N/A'}</div>
                    </div>
          ...
          <div className="drawer-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="drawer-profile">
                <div className="drawer-avatar" style={{ overflow: 'hidden' }}>
                  {selectedEmployee.profilePicture ? (
                    <img
                      src={`http://localhost:8080/uploads/${selectedEmployee.profilePicture}`}
                  ...
                  <div style={{ display: selectedEmployee.profilePicture ? 'none' : 'flex', width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
                    {getInitials(`${selectedEmployee.firstName} ${selectedEmployee.lastName}`)}
                  </div>
                </div>
                <div>
                  <h2 style={{ fontSize: '1.25rem', margin: '0 0 0.25rem 0' }}>{`${selectedEmployee.firstName} ${selectedEmployee.lastName}`}</h2>
            ...
            <div className="drawer-body">
              <div className="preview-details">
                <div className="detail-row"><span className="detail-label">Status</span><span className={`badge badge-present`} style={{ alignSelf: 'flex-start' }}>Active</span></div>
                <div className="detail-row"><span className="detail-label">Employee ID</span><span className="detail-value">{selectedEmployee.loginId}</span></div>
                ...
                <div className="detail-row"><span className="detail-label">Phone</span><span className="detail-value">{selectedEmployee.phone || 'N/A'}</span></div>
                <div className="detail-row"><span className="detail-label">Department</span><span className="detail-value">{selectedEmployee.department || 'N/A'}</span></div>
                <div className="detail-row"><span className="detail-label">Manager</span><span className="detail-value">{selectedEmployee.manager || 'N/A'}</span></div>
                <div className="detail-row"><span className="detail-label">Location</span><span className="detail-value">{selectedEmployee.location || 'N/A'}</span></div>
                <div className="detail-row"><span className="detail-label">Joining Date</span><span className="detail-value">{selectedEmployee.joiningDate || 'N/A'}</span></div>
              </div>
            </div>
            ...
            <div className="drawer-footer">
              ...
              {(user?.role === 'ADMIN' || user?.role === 'Admin' || user?.role === 'HR Officer') && (
